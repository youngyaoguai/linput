#'This package was built by Yunhan DU(1270950448@qq.com) & Chen Liang(liangchen139@163.com),from Southwestern University of Finance and Economics,China,Sichuan Province===Beijing Forestry University,Beijing.
...
linput <- function(d,p,y,x,m){
  ...
  if(extrapolate == 0 | extrapolate == 1){
    ...
    original_data <<- d
    test_data2 <<- d
    original_data$individual <<- p
    
    test_data2$individual <<- p
    original_data$year <<- as.numeric(y)
    test_data2$year <<- as.numeric(y)
    test_data2$testemploy <<- as.numeric({{x}})
    test_data2$individualID <<- as.numeric(as.factor(p))
    ...
    test_data2$individualID <<- as.numeric(as.factor(test_data2$individual))
    test_data2 <<- arrange(test_data2,individual,year,individualID)
    test_data2$individualID2 <<- 1:length(test_data2$individual)
    test_data2$individualID2 <<- as.numeric(test_data2$individualID2)
  ...
  data_totol <<- right_join(original_data,inputed_data,by = c("individual","year"))
  data_totol <<- data_totol %>%
                 select(individual,year,!c(individual,year)) %>%
                 arrange(individual,year)
  rm(original_data,envir=.GlobalEnv)
  rm(inputed_data,envir=.GlobalEnv)
  rm(countttt,envir=.GlobalEnv)
  rm(testemploy1,envir=.GlobalEnv)
  View(data_totol)
  }
